...
#endif
#define MODEM_VER_STR "Libra_Leo_signed_"
#define MODEM_VER_STR_LEN 17
#define MODEM_VER_BUF_LEN 25
...
Value * VerifyModemFn(const char *name, State *state, const std::vector<std::unique_ptr<Expr>>& argv) {
    ...
    if (ret) {
        return ErrorAbort(state, kVendorFailure,
                "%s() failed to read current MODEM build date: %d", name, ret);
    }
    ...
    for (i = 0; i < argv.size(); i++) {
        uiPrintf(state, "Checking for MODEM build date %s\n", modem_version[i].c_str());

        memset(&tm2, 0, sizeof(tm));
        strptime(modem_version[i].c_str(), "%Y%m%d", &tm2);
...
void Register_librecovery_updater_zenfone3_zoom() {
    RegisterFunction("zenfone3_zoom.verify_modem", VerifyModemFn);
}
